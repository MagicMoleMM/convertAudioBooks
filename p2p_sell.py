    ...
    for paytype in paytypes:
        ...
        for column in columns:
            ...
            if row != []:
                ...
                text = f'Лучшая продажа - {active} по цене {price_}, платежный метод - {pay_method}, биржа - {column_},  {status}. Profit {profit} / Deposit {deposit}. {status_}'
                display(text)
        '''
        with pd.ExcelWriter("/Users/magicmole/Documents/Binance/P2P.xlsx", mode='a', if_sheet_exists='replace', engine="openpyxl") as writer:  
            dt_pd.to_excel(writer, sheet_name=f"Вывод {paytype}")
