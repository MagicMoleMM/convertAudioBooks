...
if __name__ == "__main__":
    ...
    for item in data.proxies:
        # dt = ('{}:{}'.format(item.ip, item.port))
        item_ip = item_ip + [item.ip]
        item_port = item_port + [item.port]
        dt = dt + ['{}:{}'.format(item.ip, item.port)]
...
for i in range(30):
    ...
    proxy = random.choice(dt)
    print(proxy)
    try:

        if session.get("http://icanhazip.com", proxies={'http': 'http://' + proxy}, timeout=20).status_code == 200:

            print("Статус код 200, IP адрес:", session.get("http://icanhazip.com",
