...
for paytype in paytypes:
    ...
    dt_pd['percent'] = np.round(((dt_pd['results'] - dt_pd['spot'])/(dt_pd['results']))*100 - 1.2,decimals=2)
    dt_pd['loss'] = -dt_pd['percent'] * deposit / 100
    dt_pd['good_price'] = np.round(1.02 * dt_pd['spot'], decimals=2)

    def highlight_max(s, props=''):
        return np.where(s == np.nanmax(s.values), props, '')
    dt_pd.apply(highlight_max, props='color:white;background-color:darkblue', axis=0)
